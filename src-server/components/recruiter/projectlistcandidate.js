...
module.exports = (app) => {
  const db = app.get('db');
  const { project_list_candidate } = db;
  const module = {};
  ...
  module.create = async (user, row) => {
    ...
    delete row.id;
    return project_list_candidate.save({ ...row, user_id: user.id });
  };

  module.delete = async (id) => {
    if (!Number(id)) throw new Error('No id given');
    return project_list_candidate.destroy({ id });
  };
  ...
  module.get = async id => db.query(`select * from users u inner join project_list_candidate plc on u.id = plc.candidate_id where plc.project_list_id = ${id}`);
