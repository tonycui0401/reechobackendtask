...
module.exports = (app) => {
  ...
  module.create = async (user, row) => {
    ...
    delete row.id;
    return job_memo_share.save({ ...row, user_id: user.id });
  };
  ...
  module.delete = async (id) => {
    if (!Number(id)) throw new Error('No id given');
    return job_memo_share.destroy({ id });
  };
  ...
  module.get = async id => db.query(`select * from job_memo jm inner join job_memo_share jms on jms.job_memo_id = jm.id where jms.receipt_id = ${id}`);
