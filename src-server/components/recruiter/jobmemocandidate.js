...
module.exports = (app) => {
  ...
  module.create = async (user, row) => {
    ...
    delete row.id;
    return job_memo_candidate.save({ ...row, user_id: user.id });
  };
  ...
  module.delete = async (id) => {
    if (!Number(id)) throw new Error('No id given');
    return job_memo_candidate.destroy({ id });
  };

  module.getFromJobId = async id => db.query(`select * from job_memo jm inner join job_memo_candidate jmc on jm.id = jmc.job_memo_id inner join users u ON jmc.candidate_id=u.id where jm.id = ${id}`);

  // module.getFromUserId = async id => db.query(`select * from job_memo jm inner join job_memo_candidate jmc on jm.id = jmc.job_memo_id inner join users u  ON u.id = jmc.user_id where jmc.user_id = ${id}`);


  module.getFromUserId = async id => db.query(`select * from job_memo jm inner join job_memo_candidate jmc ON jm.id = jmc.job_memo_id where jmc.candidate_id = ${id}`);
